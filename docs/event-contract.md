...
| 사용량 | `Meter` | 수량·단위 |
|---|---|---|
| VM 독점 확보 | `Compute Usage` | 점유 초·`Second` (실행 여부와 무관) |
...
- 점유 경계 신호는 내부 자원 할당 측에서 제공하며 사용량 이벤트와 구분한다. VM 출처는 재점유 시에도 유지하고 유휴 중에는 정상 사용량 이벤트를 발행하지 않는다.
- 한 이벤트의 세 레코드는 자원·리전·구간이 같고 각 `Meter`가 한 번씩 존재한다.
- 모든 수량은 0 이상의 정수다.
- 같은 인스턴스의 사용 구간 중첩은 원본 보존 후 후단 품질 검증에서 탐지한다.
- 단가·통화·금액·회사·SKU와 수신 시각은 입력에 포함하지 않는다.

구간과 레코드 동일성처럼 현재 payload만으로 판정 가능한 규칙은 Kafka 소비 후 원장 적재 전 적용한다. 과거 이벤트 조회가 필요한 중첩·누락 검사는 별도 후단 품질 검증에서 다룬다.

## 6. 수신 기준

- 발생기는 Kafka에 직접 발행하고 VM `source`를 key로 사용한다. 브로커 내구성 ACK를 수신 성공으로 본다. HTTP `202` 계약은 ADR-010으로 대체한다.
- VM별 토픽을 만들지 않고 사용량 공유 토픽 하나를 사용한다. 점유 제어 토픽은 별도로 유지하며 파티션 수는 이 결정과 구분한다.
- broker 인증·쓰기 권한과 검증 가능한 발생기–source 연결이 필요하다. key·payload 자체 선언은 신원 증명이 아니다. 구체적 보안 방식은 구현 전 결정한다.
- Kafka 기록 시점에는 미검증 데이터다. 소비 단계에서 계약·출처를 검증하고 실패한 이벤트는 격리한다. ClickHouse 직접 소비로 이를 충족하는 방법은 검증 전이다.
- 거부 이력에는 원본 payload를 복사하지 않고 Kafka 위치·식별 가능한 출처·사유를 남긴다. 원본의 보관·재검토 범위는 Kafka 보관 기간에 따른다.

현재 API·OpenAPI 및 발생기 인증 코드는 이전 경로다. 직접 발행 전환 구현과 함께 제거·갱신한다.
