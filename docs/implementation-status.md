...
2026-09-25 [HLD](occupancy-attribution-hld.md): 워커 하나에 두 모듈을 배치하는 구조를 승인했다. LLD가 남아 아래 설계 상태는 `부분`, 구현·검증은 `없음`으로 유지한다.

설계: [점유 복구 ADR][occupancy-adr]·[저장소 계약][storage]·[논리 모델][logical]. 점유 이력·사용량 귀속은 워커 내부 모듈이며, [공개 안전성 계약](occupancy-attribution-contract.md)의 의미를 결정했다. 정확한 입력 스키마·원자적 처리의 세부 설계는 남았다. 실제 VM 제어는 외부 범위다.
...
| 귀속 오류·알림·차단·재처리 | 부분 | 없음 | 없음: 오류 저장·운영 조치·국소 보류 해제 미구현 |
...
따라서 수집의 ACK·중복 제거를 재설계할 필요는 없다. 공개 안전성의 기획 계약과 [테스트 설계](architecture-test-plan.md)는 마련했으며 미구현이다. 다음 판단은 **워커 LLD**다. 이후 정확한 입력 스키마·동일 버전 읽기·공개 경합·복구를 LLD로 구체화한다. 문서 결정이 새 기능 구현 승인을 뜻하지 않는다.
