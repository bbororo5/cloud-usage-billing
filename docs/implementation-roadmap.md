...
**단계 2의 테넌트 비인지 수집을 완료하고 단계 3의 귀속 설계·학습을 시작할 시점**이다. 계약·수신 API·Kafka·ClickHouse 원시 원장이 ADR-006·008과 일치한다.
...
다음 논의는 점유 이력 접근 방식이다. PostgreSQL 직접 묶음 조회와 ClickHouse 사본을 비교하며 아직 선택하지 않았다. 새 60초 규칙과 귀속 오류 처리의 코드·테스트 반영은 남아 있다.
