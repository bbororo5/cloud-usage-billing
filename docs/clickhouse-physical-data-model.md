...
> 상태: 원시 원장 기반은 구현됨. 중복 제거용 일반 뷰는 설계 승인·구현 전이다. 귀속 조회 모델의 물리 형태와 ClickHouse 직접 소비는 설계·검증 전이다. 아래 표는 구현 상태를 보증하지 않는다.
...
| 대상 | 선택 | 근거 |
|---|---|---|
| 원시 사용량 원장 | `MergeTree` 전달 레코드 (테넌트 비인지) | 발생기 계약을 신뢰해 전달 사본을 보존하고 쿼리/귀속에서 명시적으로 중복 제거한다. 회사 식별자를 갖지 않는다. |
...
| 논리 사용량 읽기 | 내부 전용 일반 `VIEW` | 중복 제거 SQL을 한곳에서 공유한다. 결과 사본·갱신 작업은 두지 않고 읽을 때 계산한다. |
...
- 동일 논리 키의 내용은 발생기가 유지한다. 기존 `payload_hash` 열은 남지만 별도 충돌 검사는 현재 수집 범위에서 제외한다.
- 원시 원장에는 `billing_account_id`를 저장하지 않으며, BFF 계정의 접근을 원천 차단한다.
- 일반 뷰에서 논리 키별 최대 `(kafka_partition, kafka_offset)` 행을 선택하여 중복을 제거한다. 적재 전 제거하거나 원시 행을 삭제하는 방식은 아니다.
- 소비 완료는 적재 묶음의 디스크 동기화 이후다. 실제 설정·실패 동작은 [저장소 계약](storage-access-contract.md#3-이벤트-적재)에 따라 검증하며 현재 SQL에 적용 완료된 것은 아니다.

### 중복 제거용 일반 뷰

원시 원장 → 일반 뷰로 논리 레코드 하나 선택 → 회사 귀속 → 비용 계산 순서다. 적재 묶음이 달라도 같은 `source + id + meter`는 한 번만 반영한다.
...
- 이는 규칙 공유를 위한 기준선이지 성능 보장이 아니다. 출처·기간 필터의 실행 계획, 읽은 행 수·메모리·p95를 측정하고 목표 미달 시 선집계를 재검토한다.
- 이번 선택은 중복 제거용 읽기 경로에 한정한다. Kafka 수신 데이터를 변환·적재하는 Materialized View의 역할과는 별개다.
