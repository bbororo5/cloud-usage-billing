...
- VM은 `유휴 → 점유 → 유휴`를 반복하며 식별자는 유지한다. 점유 시작은 새 이력을 추가하고 종료는 해당 이력을 닫는 별도 사건이다. 각 변경은 VM별 이력 버전을 증가시킨다. 캐시는 재생성 가능한 사본이며 PostgreSQL의 소유권을 대체하지 않는다.
- 예약으로 독점 확보하면 점유가 시작된다. Compute는 확보 시간을 측정하며 종료 효력 시각은 캐시 갱신 완료 시각과 구분한다.
...
- 사용 구간은 시작 포함·종료 제외이며 정확히 60초다. 점유 시작을 기준으로 측정하고 종료·점유 변경 시 잔여 구간은 발행·과금하지 않는다.
- 같은 VM의 사용 구간 전체를 포함하는 점유 이력이 정확히 하나이고, 해당 사용 구간과 겹치는 다른 점유 이력이 없어야 귀속한다. 미귀속·점유 충돌은 임의 분배하지 않고 격리한다.
- 같은 리소스의 서로 다른 논리 이벤트는 사용 구간이 겹치지 않는다.
- 각 사용량은 0 이상의 정수이며 계약에 정의된 단위를 사용한다.
- 물리적으로 중복 적재돼도 하나의 논리 `RawUsageRecord`로만 계산한다.
- 미해결 미귀속(`AttributionError`) 사용량이 존재하는 월은 월간 확정을 수행할 수 없다.
- 귀속 오류는 원본 식별자·VM source·사용 구간·사유와 해결 이력을 보존한다. 담당자에게 알리고 점유 이력 수정 후 원본을 재처리하며, 해결 전 해당 사용량은 공개하지 않는다.
