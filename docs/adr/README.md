...
| [007](0007-usage-ledger-writer-execution-unit.md) | Kafka에서 ClickHouse로 원장을 적재하는 실행 단위 | 물리 데이터 흐름 확정 후 | ADR-010으로 부분 대체·이전 코드 보관 |
...
ADR 본문은 결정 당시 비교 근거를 보존한다. 이후 변경은 각 상태란에 표시하며, 최신 구조·미구현 범위는 [구현 로드맵](../implementation-roadmap.md)을 기준으로 한다.
